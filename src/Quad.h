...
template <typename T> std::string Quad<T>::to_string(const bool& spread, const bool& full_info, const unsigned int& indent) const {
	...
	if (full_info){
		...
		//mes+=(m_pTLTree==NULL)?nulll : m_pTLTree->to_string(spread, indent, full_info)+", ";
		//mes+=(m_pTRTree==NULL)?nulll : m_pTRTree->to_string(spread, indent, full_info)+", ";
		//mes+=(m_pBLTree==NULL)?nulll : m_pBLTree->to_string(spread, indent, full_info)+", ";
		//mes+=(m_pBRTree==NULL)?nulll : m_pBRTree->to_string(spread, indent, full_info)+", ";
		mes+=(m_pTLTree==NULL)?nulll : m_pTLTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		//mes+=", ";
		mes+=(m_pTRTree==NULL)?nulll : m_pTRTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		//mes+=", ";
		mes+=(m_pBLTree==NULL)?nulll : m_pBLTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		//mes+=", ";
		mes+=(m_pBRTree==NULL)?nulll : m_pBRTree->to_string(spread, indent+1, full_info);
