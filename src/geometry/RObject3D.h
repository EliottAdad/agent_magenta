 ...
 * ###################
 */
template<typename M, typename E> class RObject3D : public Object3D<M, E> {
public:
	std::shared_ptr<SN<M, E>> pangle;
	std::shared_ptr<Shape3D<M, E>> pcollision_box;										// Collision box.

	RObject3D();//:)
	RObject3D(std::string name, SDL_Color* pcolor=NULL);//:)
	RObject3D(const RObject3D& robject);//:)
	~RObject3D();
	...
	void operator=(const RObject3D<M, E>& robject);//:)
	void operator+=(const RObject3D<M, E>& robject);//:)
	void operator-=(const RObject3D<M, E>& robject);//:)
	//void operator*=(double k);//:)
};

template<typename M, typename E> bool operator==(const RObject3D<M, E>& robject1, const RObject3D<M, E>& robject2);//:)

template<typename M, typename E> bool testCollide(const RObject3D<M, E>& robject1, const RObject3D<M, E>& robject2);//:)

//void print(RObject2D* probject, unsigned int indent=0);//:)


template<typename M, typename E> RObject3D<M, E>::RObject2D(){
 ...
 */
template<typename M, typename E> RObject3D<M, E>::RObject3D(std::string name, SDL_Color* pcolor) {
	//printf("ROBJECT: CONSTRUCTOR START\n");
	m_name=name;
	m_mass={1, 1};
	m_charge={0, 0};
...
template<typename M, typename E> RObject3D<M, E>::RObject3D(const RObject3D& robject) {
 ...
 */
template<typename M, typename E> RObject3D<M, E>::~RObject3D() {
...
template<typename M, typename E> void RObject3D<M, E>::move(long double dt) {
	if (m_fmove)
	{
...
template<typename M, typename E> void RObject3D<M, E>::spin(long double dt) {
...
template<typename M, typename E> void RObject3D<M, E>::operator=(const RObject3D<M, E>& robject) {
	//printf("ROBJECT op= 1\n");
