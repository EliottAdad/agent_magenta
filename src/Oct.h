...
template <typename T> std::string Oct<T>::to_string(const bool& spread, const bool& full_info, const unsigned int& indent) const {
	...
	if (full_info){
		...
		mes+=(m_pBLBTree==NULL)?nulll : m_pBLBTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pBRBTree==NULL)?nulll : m_pBRBTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pBRFTree==NULL)?nulll : m_pBRFTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pBLFTree==NULL)?nulll : m_pBLFTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pTLBTree==NULL)?nulll : m_pTLBTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pTRBTree==NULL)?nulll : m_pTRBTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pTRFTree==NULL)?nulll : m_pTRFTree->to_string(spread, indent+1, full_info);
		mes+=((spread)?to_stringTabs(indent+1) : "");
		mes+=(m_pTLFTree==NULL)?nulll : m_pTLFTree->to_string(spread, indent+1, full_info);
		//mes+=((spread)?to_stringTabs(indent+1) : "");
